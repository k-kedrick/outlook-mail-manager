...
4. **导出重做**：`导出` 打开列勾选弹窗（账号/密码/clientId/刷新令牌/卡密/2FA密钥），后端 `accounts/export` 按 `?fields=` 白名单取列、**按固定列序**用 `----` 拼接、**空字段输出字面量 `error`**、**行序按 `createdAt` 升序**（与列表显示顺序一致；见第九期，早期版本曾按邮箱升序 / 按传入 id 顺序）。
...
2. **导出所选顺序**：曾一度改为「按前端传入 id 顺序输出」，但前端选中集合是 `Set`（逐个点选按点击先后插入），导致导出行序跟着点击顺序乱跳。第九期已改为**一律按 `createdAt` 升序**，与列表显示顺序完全一致，不再受勾选先后或是否跨页影响。
...
**第九期 · 导出行序修复（当前最新，已验证）**
- **导出行序不再跟着勾选顺序乱跳**：根因是前端选中集合用 `Set` 存，逐个点选按点击先后插入（表头全选才按列表顺序），导出取 `Array.from(selected)` 保留插入顺序，后端 `accounts/export` 又按传入 `ids` 顺序（`ids.indexOf`）重排，于是「逐个点选」导出的行序 = 点击先后，不可预测。**已改为后端 `accounts/export` 一律 `orderBy: createdAt asc`、删除按 `ids` 的重排**，导出行序恒等于列表显示顺序，与勾选先后、是否跨页无关。字段仍按 `FIELD_ORDER` 固定列序、空字段仍输出 `error`、`ids` 仍只用于筛选选中账号。导出弹窗提示文案同步改为「按列表顺序（导入/创建顺序）排列」。
